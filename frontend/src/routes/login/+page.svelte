...
<main class="auth-page">
  <section class="auth-art"><div class="glow"></div><div class="art-brand"><BrandLogo variant="hero" href="/login" /></div><div class="art-copy"><h1>Información que impulsa empresas.</h1></div></section>
    ...
    <form method="POST" use:enhance={() => { submitting = true; return async ({ update }) => { await update(); submitting = false; }; }}>
      ...
      {#if form?.error}
        <div class="form-error" role="alert">
          ...
          {#if form.errorDetail}<span>{form.errorDetail}</span>{/if}
          {#if form.errorCode}<code>{form.errorCode}</code>{/if}
        </div>
      {/if}
